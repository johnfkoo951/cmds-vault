...
> 2. **Vault operator** (이 볼트의 사용자): 시작 시점에는 *user-scope* 노트에 `[[Me]]` placeholder. WELCOME ritual 후 본인 정보로 채워짐. 시스템 파일 author는 `[[구요한]]` (upstream attribution, 변경 X).
...
> 4. **Author 표기 (두 가지 정체)**:
>    - **시스템 파일** (CLAUDE/AGENTS/CMDS/Guide/HQ/WELCOME/README, 슬래시 커맨드, rules, skills): `author: [[구요한]]` (upstream attribution — 변경 X)
>    - **사용자 작성 노트** (BRAIN.md, daily notes, permanent notes, 캡처): `author: [[Me]]` placeholder → WELCOME ritual로 본인 이름으로 치환
...
This vault has **6 core system files** that complement each other. You are currently reading the **context guide**.
...
| File          | Your Use Case           | Key Content                                 |
| ------------- | ----------------------- | ------------------------------------------- |
| **DESIGN.md** | Making visual artifacts | CI color tokens, Anti-Slop rules, templates |
